...
func main() {
	...
	microService:= web.NewService(
		web.Name("api.tutor.com.bottleserver"),
...
func init() {
	...
	initConfig()
	//初始化日志
	mylog.InitLog(config.LogPath,"bottleserver", config.LogMaxAge, config.LogMaxSize, config.LogMaxBackups, int8(config.LogLevel))
	//初始化token
